...
3. **Runtime render of data.** Confirm the dev server is up (the /clepo command starts it on
   http://localhost:5175; if not, start `npm run dev` in the background and poll the port). Then fetch a
   couple of routes' HTML (`curl -s http://localhost:5175/` and `/menu`) and confirm data-driven text
   actually appears in the DOM — i.e. the mockup data is rendering, not blank.
